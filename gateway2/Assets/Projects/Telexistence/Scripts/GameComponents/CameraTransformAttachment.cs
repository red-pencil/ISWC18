...
public class CameraTransformAttachment : MonoBehaviour {
	...
	void Update () {
        ...
        if (renderer.BodyController == null || renderer.BodyController.RobotJointValues.Length < 6 || !EnableHeadMatching || renderer.TrackingMode == TxEyesRenderer.Mode.OculusSync)
        {
            ...
            _angleAverage[0].Sample(renderer.BodyController.RobotJointValues[1]);
            _angleAverage[1].Sample(renderer.BodyController.RobotJointValues[3]);
            _angleAverage[2].Sample(renderer.BodyController.RobotJointValues[5]);
