...
*/
	...
	void ReadAudio(float[] data, int channels)
	{
		...
		while (length < DataLength) {
			...
			average+= GstNetworkAudioPlayer.ProcessAudioPackets(p.samples, (int)p.startIndex, (int)channelIndex, (int)count, (int)stepSize, (int)srcChannelsCount, data, (int)length,(int) channels);
