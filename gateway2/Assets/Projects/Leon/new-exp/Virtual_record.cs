...
//			}
	...
	void Record(){
		...
		writer.AddData ("Time-Time", Time.time.ToString ());
		writer.AddData ("Target ID", targetIDreal.ToString ());
		writer.AddData ("HMD_rx", vOrigin.transform.rotation.eulerAngles.x.ToString ());
		writer.AddData ("HMD_ry", vOrigin.transform.rotation.eulerAngles.y.ToString ());
		writer.AddData ("HMD_rz", vOrigin.transform.rotation.eulerAngles.z.ToString ());
//		writer.AddData ("Target_x", targetx.ToString ());
//		writer.AddData ("Target_y", targety.ToString ());
//		writer.AddData ("Target_z", targetz.ToString ());
...
//		writer.AddData ("Target_lat", lat[vtOrder[targetID-1] -1].ToString ());
    ...
    void RandomTarget(){
		...
		for (int i = 1; i < (vtNumber + 1); i = i + 0) { // i = 1~10
			targetIDtemp = Random.Range (1, (vtNumber + 1)); // exclusive = 1~10 don't use 0, because initial value is 0
