...
public class recordData : MonoBehaviour {
	...
	//	InvokeRepeating("OptiTrack", 0.0f, 1.0f/(float)SamplingSpeed);
		...
		writer.AddKey ("CameraY");
		writer.AddKey ("HeadY");
		writer.AddKey ("CameraX");
		writer.AddKey ("HeadX");
		writer.AddKey ("CameraZ");
		writer.AddKey ("HeadZ");
    ...
    void OnDestroy()
    {
        if (_started)
        {
            writer.WriteValues(Application.dataPath + "/data00.txt");
        }
    }
	...
	void OptiTrack () {
		...
		*/
		...
		writer.AddData ("CameraY", camerapos.y.NormalizeAngle().ToString ());
