...
public abstract class IGstPlayer {
    ...
    [DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
    extern static private int mray_gst_PlayerRTPGetEyeGazeCount(System.IntPtr p, int index);
	[DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
    extern static private int mray_gst_PlayerRTPGetEyeGazeLevels(System.IntPtr p, int index, int eye);
	[DllImport(DllName, CallingConvention = CallingConvention.Cdecl)]
    extern static private void mray_gst_PlayerRTPGetEyeGazeData(System.IntPtr p, int index, int eye, int level, ref int x, ref int y, ref int w, ref int h);
    ...
    public int RTPGetEyegazeCount(int index)
    {
        return mray_gst_PlayerRTPGetEyeGazeCount(m_Instance, index);
    }
	public int RTPGetEyegazeLevels(int index,int eye)
	{
        return mray_gst_PlayerRTPGetEyeGazeLevels(m_Instance, index, eye);
	}
    public Vector4 RTPGetEyegaze(int index, int eye, int level)
	{
		int x=0, y=0, w=0, h=0;
        mray_gst_PlayerRTPGetEyeGazeData(m_Instance, index, eye, level, ref x, ref y, ref w, ref h);
