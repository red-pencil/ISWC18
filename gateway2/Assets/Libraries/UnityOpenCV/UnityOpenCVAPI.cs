...
public class UnityOpenCVFaceDetectorAPI {
	...
	[DllImport(DllName)]
	private static extern IntPtr FaceDetector_Create([MarshalAs(UnmanagedType.LPStr)]string cascadesPath,float scaler,int minNeighbors,float minSize,float maxSize);
	...
	public UnityOpenCVFaceDetectorAPI(string cascadesPath,float scaler=1.01f,int minNeighbors=8,float minSize=1.0f/5.0f,float maxSize=2.0f/3.0f)
	{
		_instance = FaceDetector_Create (cascadesPath,scaler,minNeighbors,minSize,maxSize);
	}
